...
export function AdminSidebar({ email, role }: Props) {
      ...
      {NAV.map(({ href, label, icon: Icon, exact }) => {
        ...
        return (
          <Link
            ...
            onClick={onClick}
            className={cn(
              "inline-flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition",
              active
  ...
  return (
    <>
      ...
      {/* Desktop sidebar */}
      <aside className="sticky top-0 hidden h-[100dvh] w-64 shrink-0 flex-col border-r border-white/10 bg-bg-surface md:flex">
